...
public class AmbariClusterTemplateGeneratorTest {
    ...
    public void testDefaultPasswordIsUserProvided() throws CloudbreakException {
        ...
        Mockito.verify(ambariClient, Mockito.times(1)).
                createClusterJson(ArgumentMatchers.anyString(), ArgumentMatchers.any(), ArgumentMatchers.eq("UserProvidedPassword"), ArgumentMatchers.eq(ConfigStrategy.ALWAYS_APPLY_DONT_OVERRIDE_CUSTOM_VALUES.name()),
                        ArgumentMatchers.isNull(), ArgumentMatchers.isNull(), ArgumentMatchers.isNull(), ArgumentMatchers.anyBoolean(), ArgumentMatchers.isNull());
    }
    ...
    public void testDefaultPasswordIsUserProvidedSecure() throws CloudbreakException {
        ...
        Mockito.verify(ambariClient, Mockito.times(1)).
                createClusterJson(ArgumentMatchers.anyString(), ArgumentMatchers.any(), ArgumentMatchers.eq("UserProvidedPassword"), ArgumentMatchers.eq(ConfigStrategy.ALWAYS_APPLY_DONT_OVERRIDE_CUSTOM_VALUES.name()),
                        ArgumentMatchers.eq("principal"), ArgumentMatchers.eq("KerberosPassword"), ArgumentMatchers.eq("PERSISTED"), ArgumentMatchers.anyBoolean(), ArgumentMatchers.isNull());
    }
